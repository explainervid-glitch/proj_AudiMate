# ============================================================
# AudiMate v1.5.0 - Installer Script (PowerShell)
...
if (-not $isAdmin) {
    ...
    # drive (e.g. Z:\...), that drive letter is NOT visible inside
    ...
    # and the AudiMate\ plugin folder) to a local temp folder BEFORE
    # elevating, then elevate and run from there. The elevated
    # session never needs to touch the network drive at all.
    # ------------------------------------------------------------
    $localPreStage = "$env:TEMP\AudiMate_preelevate"
    ...
    try {
        ...
        Write-Err "Make sure the network drive (Z:) is accessible."
        Safe-Exit 1
    }
    ...
    $tempLauncher = "$env:TEMP\AudiMate_elevate_launcher.ps1"
...
Write-Host "   AudiMate v1.5.0 - Installer for Adobe Animate   " -ForegroundColor Cyan
Write-Host "  ==================================================" -ForegroundColor DarkCyan
Write-Host ""
...
$sourceDir = Join-Path $scriptDir "AudiMate"

$destSystem = "C:\Program Files (x86)\Common Files\Adobe\CEP\extensions\AudiMate"
$destUser   = "$env:APPDATA\Adobe\CEP\extensions\AudiMate"
...
if (-not (Test-Path "$sourceDir\CSXS\manifest.xml")) {
    Write-Host ""
    Write-Err "Folder 'AudiMate' or manifest.xml not found."
    ...
    Write-Host "    AudiMate\"                 -ForegroundColor Gray
    Write-Host "      CSXS\manifest.xml"      -ForegroundColor Gray
    Write-Host "      AudiMate.html"          -ForegroundColor Gray
    Write-Host "      css\style.css"          -ForegroundColor Gray
    Write-Host "      js\main.js"             -ForegroundColor Gray
    Write-Host "      jsx\AudiMate.jsx"       -ForegroundColor Gray
    Write-Host ""
    Safe-Exit 1
}
...
$checks = @(
    ...
    @{ Path = "$destDir\AudiMate.html";     Label = "AudiMate.html" },
    @{ Path = "$destDir\jsx\AudiMate.jsx";  Label = "AudiMate.jsx"  },
    @{ Path = "$destDir\css\style.css";     Label = "style.css"     },
    @{ Path = "$destDir\js\main.js";        Label = "main.js"       }
)
...
if ($allOk) {
    ...
    Write-Host "  1. Open Adobe Animate 2024"                    -ForegroundColor Gray
    Write-Host "  2. Panel   : Window > Extensions > AudiMate"   -ForegroundColor Gray
    Write-Host ""
